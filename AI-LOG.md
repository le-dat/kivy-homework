...
| **Độ trễ Webhook & Chống ghi đè trạng thái** | Dùng Postman gửi liên tiếp 2 request cập nhật trạng thái cho cùng 1 bản ghi: Request 1 báo `VERIFIED`, Request 2 (giả lập webhook đến muộn) báo `REJECTED`. | Database chặn thành công Request 2 nhờ câu lệnh UPDATE có check `status NOT IN (terminal_states)`. Bản ghi giữ nguyên `VERIFIED` và trả về HTTP 400. |
